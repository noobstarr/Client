...
import com.jagex.rs2.node.MRUNodes;
...
public class InterfaceListener {
	...
	public static void unpack(StreamLoader streamLoader, TextDrawingArea textDrawingAreas[], StreamLoader streamLoader_1) {
		...
		interfaceCache = new InterfaceListener[j];
		while (stream.currentOffset < stream.buffer.length) {
			...
			InterfaceListener rsInterface = interfaceCache[k] = new InterfaceListener();
			rsInterface.id = k;
		...
		Interface.load(streamLoader, textDrawingAreas);
		aMRUNodes_238 = null;
	}
	...
	public static void addConfigButton(int ID, int pID, int bID, int bID2, String bName, int width, int height, String tT, int configID, int aT, int configFrame) {
		InterfaceListener tab = addTabInterface(ID);
		tab.parentID = pID;
		tab.id = ID;
		tab.type = 5;
		tab.atActionType = aT;
	...
	{
		InterfaceListener rsi = addInterface(id);
	...
	public static InterfaceListener addInterface(int id)
	{
		InterfaceListener rsi = interfaceCache[id] = new InterfaceListener();
		rsi.id = id;
		rsi.parentID = id;
		rsi.width = 512;
		rsi.height = 334;
	...
	public static void setBounds(int ID, int X, int Y, int frame, InterfaceListener RSinterface){
	...
	public static void textSize(int id, TextDrawingArea tda[], int idx) { 
		InterfaceListener rsi = interfaceCache[id]; rsi.textDrawingAreas = tda[idx]; 
	}
	...
	public static InterfaceListener addTabInterface(int id) {
		InterfaceListener tab = interfaceCache[id] = new InterfaceListener();
		tab.id = id;//250
		tab.parentID = id;//236
		tab.type = 0;//262
	...
	public Model method209(int j, int k, boolean flag) {
		...
		if (k == -1 && j == -1 && model.anIntArray1640 == null)
			return model;
		Model model_1 = new Model(true, AnimationHeader.method532(k) & AnimationHeader.method532(j), false, model);
		if (k != -1 || j != -1)
			model_1.method469();
		if (k != -1)
			model_1.method470(k);
	...
	public static void addNewSprite(int id, int sprite)
	{ InterfaceListener class9 = interfaceCache[id];
	...
	public static InterfaceListener interfaceCache[];
